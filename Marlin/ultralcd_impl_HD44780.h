...
void lcd_print(const char *str) {
	while (*str) lcd.print(*str++);
}
void lcd_printPGM(const char *str) {
	while (const char c = pgm_read_byte(str)) lcd.print(c), ++str;
}
...
FORCE_INLINE void _draw_heater_status_1(const int8_t heater, const char prefix, const bool blink) {
	const bool isBed = heater < 0;
...
static void lcd_implementation_status_screen() {
	...
	//
	_draw_heater_status_1(0, LCD_STR_THERMOMETER[0], blink);
...
#else
	_draw_heater_status_1(1, LCD_STR_THERMOMETER[0], blink);
#endif
	...
	_draw_heater_status_1(-1, LCD_BEDTEMP_CHAR, blink);
#endif // HOTENDS > 1 || TEMP_SENSOR_BED != 0
...
FORCE_INLINE static void clear_custom_char(custom_char * const cc) {
	ZERO(cc->custom_char_bits);
}
...
inline coordinate pixel_location(const uint8_t x, const uint8_t y) {
	return pixel_location((int16_t) x, (int16_t) y);
}
